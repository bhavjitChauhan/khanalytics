const { Router } = require('express');

require('module-alias/register');
const cache = require('@middlewares/cache');
const { find } = require('@lib/mongo');
const { HOUR, DAY, WEEK, MONTH, YEAR } = require('@util/durations');


const router = Router();

router.get('/hour', cache, async (_req, res) => {
    const data = await find('hotlist', { timestamp: { $gt: Date.now() - HOUR } });
    res.send(data);
});

router.get('/hours/:hours', cache, async (req, res) => {
    const { hours } = req.params;
    const data = await find('hotlist', { timestamp: { $gt: Date.now() - HOUR * hours } });
    res.send(data);
});

router.get('/day', cache, async (_req, res) => {
    const data = await find('hotlist', { timestamp: { $gt: Date.now() - DAY } });
    res.send(data);
});

router.get('/days/:days', cache, async (req, res) => {
    const { days } = req.params;
    const data = await find('hotlist', { timestamp: { $gt: Date.now() - DAY * days } });
    res.send(data);
});

router.get('/week', cache, async (_req, res) => {
    const data = await find('hotlist', { timestamp: { $gt: Date.now() - WEEK } });
    res.send(data);
});

router.get('/weeks/:weeks', cache, async (req, res) => {
    const { weeks } = req.params;
    const data = await find('hotlist', { timestamp: { $gt: Date.now() - WEEK * weeks } });
    res.send(data);
});

router.get('/month', cache, async (_req, res) => {
    const data = await find('hotlist', { timestamp: { $gt: Date.now() - MONTH } });
    res.send(data);
});

router.get('/months/:months', cache, async (req, res) => {
    const { months } = req.params;
    const data = await find('hotlist', { timestamp: { $gt: Date.now() - MONTH * months } });
    res.send(data);
});

router.get('/year', cache, async (_req, res) => {
    const data = await find('hotlist', { timestamp: { $gt: Date.now() - YEAR } });
    res.send(data);
});

router.get('/years/:years', cache, async (req, res) => {
    const { years } = req.params;
    const data = await find('hotlist', { timestamp: { $gt: Date.now() - YEAR * years } });
    res.send(data);
});

router.get('/before/:latest', cache, async (req, res) => {
    const { latest } = req.params;
    const data = await find('hotlist', { timestamp: { $lte: latest } });
    res.send(data);
});

router.get('/after/:earliest', cache, async (req, res) => {
    const { earliest } = req.params;
    const data = await find('hotlist', { timestamp: { $gte: earliest } });
    res.send(data);
});

router.get('/max', cache, async (_req, res) => {
    const data = await find('hotlist');
    res.send(data);
});


module.exports = router;
